    ...
    result() {
        ...
        this.swap(`
            <div class="rx">
                ...
                ${altRow}
                <div class="rx__cta">
                    <a class="rx__btn rx__btn--solid" href="#" data-soon>
...
/* =============================================================================
   INQUIRE — question-form embed (launch build)
   -----------------------------------------------------------------------------
   Ported from the contact page so the index can carry the form on its own.
   Loads the Jotform question form into #inqFrame and lets it grow to the
   height Jotform posts, so there is no nested scrollbar.
============================================================================= */
(function () {
    const INQ_FORM = 'https://form.jotform.com/262338508765062'; // Contact Alpha Vital Elite IV

    const mount = () => {
        const frame = document.getElementById('inqFrame');
        if (!frame) return;

        if (!INQ_FORM) {                       // no URL yet -> hide the panel
            document.getElementById('inqSlot')?.setAttribute('hidden', '');
            return;
        }
...
/* =============================================================================
   UNDER-CONSTRUCTION POPUP (launch build)
   -----------------------------------------------------------------------------
   Any element marked [data-soon] opens the "coming soon" modal instead of
   navigating. Close with the X, the backdrop, or Escape. "Send us a question"
   closes the modal and scrolls to the on-page question form.
============================================================================= */
(function () {
    ...
    const open = () => {
        ...
        modal.querySelector('.soon__x')?.focus();
    };
    ...
    const close = () => {
        ...
        setTimeout(() => { if (!modal.classList.contains('is-open')) modal.hidden = true; }, 340);
        lastFocus?.focus?.();
    };

    // open on any not-ready trigger
    document.addEventListener('click', (e) => {
    ...
    document.addEventListener('click', (e) => {
        if (e.target.closest('[data-soon-close]')) { e.preventDefault(); close(); }
    });

    // "Send us a question" -> close, then scroll to the form
    modal.querySelector('[data-soon-ask]')?.addEventListener('click', (e) => {
        e.preventDefault();
        close();
        setTimeout(() => {
            const form = document.getElementById('inquire');
            if (!form) return;
            const y = form.getBoundingClientRect().top + window.scrollY - 70;
